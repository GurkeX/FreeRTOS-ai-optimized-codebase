...
- **None required** — uses project defaults
...
1. Check the existing dev build artifact exists:
   ...
   ```
2. If available, record the dev UF2 size. If it doesn't exist, note "no baseline available".
...
|------------------|-----------|------------------|-----------|
| UF2 size         | [dev KB]  | [prod KB]        | [X%]      |
| .text (code)     | [dev KB]  | [prod KB]        | [X%]      |
| .bss (zero-init) | [dev KB]  | [prod KB]        | [X%]      |
```
...
| Stack overflow detection | **KEPT** | Runtime safety net; graceful `watchdog_reboot()` on overflow |
| Malloc failed hook | **KEPT** | Runtime safety net; graceful `watchdog_reboot()` on OOM |
| Event Groups | **KEPT** | **Required by FreeRTOS SMP port** for RP2040 dual-core spinlock sync |
...
| RTT stdio + buffers | **STRIPPED** | No RTT channels in production |
| Runtime stats / trace facility | **STRIPPED** | FreeRTOS observability macros |
| Task name strings | **STRIPPED** | `configMAX_TASK_NAME_LEN = 2` in production (minimum required by kernel) |
| Queue registry | **STRIPPED** | Debug-only queue naming disabled |

## Production Firmware Behavior

- Blinky task toggles LED at hardcoded interval (`BLINKY_DELAY_MS`, no persistent config lookup)
...
| Linker errors for `xEventGroup*` | `configUSE_EVENT_GROUPS` was set to 0 | **Must be 1** — SMP port requires Event Groups; check `FreeRTOSConfig.h` |
