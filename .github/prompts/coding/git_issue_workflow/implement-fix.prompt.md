...
# Implement Fix: GitHub Issue $github-issue-id
...
-   RCA document exists at `docs/rca/issue-$github-issue-id.md`
-   GitHub CLI installed and authenticated (optional, for status updates)

## RCA Document to Reference

Read RCA: `docs/rca/issue-$github-issue-id.md`
...
-   Review the GitHub issue details (issue #$github-issue-id)
...
```python
def test_issue_$github-issue-id_fix():
    """Test that issue #$github-issue-id is fixed."""
...
-   Add notes about the fix under ./resources/fixes/fix-issue-$github-issue-id.md

## Output Report

### Fix Implementation Summary

**GitHub Issue #$github-issue-id**: [Brief title]
...
```
fix(scope): resolve GitHub issue #$github-issue-id - [brief description]
...
**Note:** Using `Fixes $github-issue-id` in the commit message will automatically close the GitHub issue when merged to the default branch.
...
```bash
gh issue comment $github-issue-id --body "Fix implemented in commit [commit-hash]. Ready for review."
```

**Update issue labels (if needed):**

```bash
gh issue edit $github-issue-id --add-label "fixed" --remove-label "bug"
```

**Close the issue (if not using auto-close via commit message):**

```bash
gh issue close $github-issue-id --comment "Fixed and merged."
```

## Notes

-   If the RCA document is missing or incomplete, request it be created first with `/rca $github-issue-id`
...
-   The commit message `Fixes $github-issue-id` will link the commit to the GitHub issue
